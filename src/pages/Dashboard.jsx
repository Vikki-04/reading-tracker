...
const styles = {
  ...
  bodyTextOnTile: { margin: '0 0 10px', color: '#fcfaf7', lineHeight: 1.5 },
  bodyTextOnTileLast: { margin: 0, color: '#fcfaf7', lineHeight: 1.5 },
  insightPanel: {
    padding: '10px 12px',
    borderRadius: 14,
    background: '#a18770',
  ...
  insightListOnTile: { margin: 0, paddingLeft: 18, color: '#fcfaf7', lineHeight: 1.6 },
...
export function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([getReadBooks(), getTBRBooks()]).then(([read, tbrData]) => {
      setReadByYear(read)
      setTbr(tbrData)
      setLoading(false)
    })
  ...
  const emptyReadByYear = useMemo(
    () => Object.fromEntries(YEAR_OPTIONS.map((y) => [y, []])),
    [],
  )

  const stats = useMemo(() => {
    const data = readByYear ?? emptyReadByYear
    const readSet = collectReadTitleSet(data)
    const counts = {}
    let total = 0
    for (const y of YEAR_OPTIONS) {
    ...
    for (const [s, c] of Object.entries(readCountBySeries)) {
      if (c > mostReadCount) { mostReadCount = c; mostReadSeries = s }
    }
    ...
    for (const [seriesName, titles] of catalog.entries()) {
      ...
      if (readIn > 0 && readIn < titles.length) {
        inProgressSeries.push({ name: seriesName, read: readIn, total: titles.length })
      }
    }
    inProgressSeries.sort((a, b) => b.read / Math.max(1, b.total) - a.read / Math.max(1, a.total))
    ...
    for (const row of inProgressSeries) {
      ...
      const nextUnread = order.find((t) => !readSet.has(normalizeTitleKey(t))) ?? onTbr[0]?.title ?? null
      activeSeries.push({ ...row, nextUnread, tbrLeft: onTbr.length })
    }

    let longestCatalogSeriesName = null
    let longestCatalogN = 0
    for (const [name, titles] of catalog.entries()) {
      if (titles.length > longestCatalogN) { longestCatalogN = titles.length; longestCatalogSeriesName = name }
    }
    ...
    if (c2026 > 0 || c2025 > 0) {
      ...
      else if (diff < 0) trendMsg = `2025 still leads 2026 by ${Math.abs(diff)} book${Math.abs(diff) === 1 ? '' : 's'}—plenty of year left.`
      else trendMsg = '2026 and 2025 are tied so far.'
    }

    const now = new Date()
    const monthIdx = now.getMonth() + 1
    const perMonth2026 = monthIdx > 0 ? Math.round((c2026 / monthIdx) * 10) / 10 : 0

    return {
      counts, total, yearsWithData, avgAmongLoggedYears,
      mostProductiveYear: bestYear,
      maxBar: Math.max(1, ...YEAR_OPTIONS.map((y) => counts[y])),
      readCountBySeries, mostReadSeries,
      mostReadCount: mostReadCount > 0 ? mostReadCount : null,
      longestCompleted, longestCompletedN: longestN,
      inProgressSeries, activeSeries,
      longestCatalogSeriesName, longestCatalogLen: longestCatalogN,
      trendMsg, perMonth2026, monthIdx,
    }
  }, [readByYear, tbr, emptyReadByYear])

  const showPhotoBg = READING_LOG_BACKGROUND_SRC.length > 0
  const pageShellStyle = { ...styles.pageShell, background: PAGE_DEFAULT_BG }
  const funLongestSeries = stats.longestCatalogLen > 0
      ...
      <div style={styles.pageContent}>
        <header style={styles.pageHeader}>
          <div style={styles.titleRow}>
            ...
            <p style={styles.badge}>📊 {stats.total} total</p>
          </div>
        ...
        {loading ? (
          <div style={styles.loading}>Loading your stats…</div>
        ) : (
          <>
            ...
            <section style={styles.section} aria-label="Activity insights">
              ...
              <div style={styles.insightPanel}>
                <ul style={styles.insightListOnTile}>
                  {stats.mostProductiveYear ? (
                    <li>
                      You logged the most books in <strong>{stats.mostProductiveYear}</strong> ({stats.counts[stats.mostProductiveYear]} books).
                    </li>
                  ) : (
            ...
            <section style={styles.section} aria-label="Active series">
              ...
              {stats.activeSeries.length === 0 ? (
                <p style={styles.emptyOnPhoto}>No partial series detected.</p>
              ) : (
                stats.activeSeries.map((row) => (
                  <div key={row.name} style={styles.seriesItem}>
                    ...
                    <p style={styles.seriesSub}>
                      ...
                      {row.nextUnread ? <> · Next up: <strong>{row.nextUnread}</strong></> : null}
                      {row.tbrLeft ? <> · {row.tbrLeft} still on TBR</> : null}
                    </p>
                  </div>
                ))
              )}
            ...
            <section style={styles.section} aria-label="Quick actions">
              ...
              <div style={styles.actionsRow}>
                <Link to="/" style={styles.actionLink}>➕ Add book</Link>
                <Link to="/" style={styles.actionLinkGhost}>📖 Reading Log</Link>
                <Link to="/tbr" style={styles.actionLinkGhost}>📚 Open TBR</Link>
              </div>
            </section>
          </>
        )}
